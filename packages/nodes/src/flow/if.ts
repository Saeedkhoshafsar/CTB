 ...
 *
 * Comparison semantics (documented, deliberately forgiving — flow authors
 * compare expression results that often arrive as strings):
...
function evalCondition(c: IfCondition): boolean {
  const { value1: a, operator, value2: b } = c;
  switch (operator) {
    case 'equals':
