...
Task      : → current: P1-T7 (MVP node set, wave 1)   [not started]
...
executor loop (P1-T4) + Telegram gateway (P1-T5) + **update router (P1-T6)** done.
The conversational core works end-to-end in tests: trigger starts an execution,
a reply resumes the WAITING one (validated, fa-digit aware, retries durable),
timeouts fire the `timeout` port, chats are isolated by a per-chat mutex.
Next: P1-T7 — the real MVP nodes (tg.trigger, tg.sendMessage, tg.waitForReply,
flow.if, data.setFields, flow.stopError) in packages/nodes.
